...
uint32_t cache_lookup_dm(struct cache_st *csp, uint64_t addr) {
    ...
    addr_word = addr >> 2;                  // Calculate the Word Address
    b_index = addr_word % csp->block_size;  // Caluclate the Block Index                                            
    b_base = addr_word - b_index;           // Calculate the Block Base
    byte_addr = b_base << 2;                // Calculate the Byte Address
    ...
    if (slot->valid && (slot->tag == tag)) {
        ...
        // Fill the block with the instruction words near the byte address
        for (int i = 0; i < csp->block_size; i++) {
            slot->block[i] = (*(uint64_t*)(byte_addr + i*sizeof(uint32_t))); // Use the byte_address + i*4 because each IW is 4 bytes long
        }
...
uint32_t cache_lookup_sa(struct cache_st *csp, uint64_t addr) {
    ...
    uint64_t addr_word = addr >> 2;                  // Calculate Word Address
    uint64_t b_index = addr_word % csp->block_size;  // Calculate Block Index
    uint64_t b_base = addr_word - b_index;           // Calculate Block Base                        
    uint64_t byte_addr = b_base << 2;             // Calculate Byte Address
    ...
    if (!slot_found) {
        if (slot_invalid) {
            ...
            // Using csp->refs which gets set to the slot->timestamp
            // smallest slot->timestimp
            
            slot = &(csp->slots[set_base]);
            for (int i = 0; i < csp->ways; i++) {
                struct cache_slot_st *tmp_slot = &(csp->slots[set_base + i]);
                if (tmp_slot->timestamp < slot->timestamp) {
                    slot = tmp_slot;
                }
            }
    ...
    if (!hit) {
        for (int i = 0; i < csp->block_size; i++) {
            slot->block[i] = *(uint32_t*)(byte_addr + i*sizeof(uint32_t));
        }       
        slot->tag = tag;
        slot->valid = true;
    }
